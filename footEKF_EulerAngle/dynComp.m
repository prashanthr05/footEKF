...
t = min([length(a.timeStamp),length(omega.timeStamp),length(torso.timeStamp),length(right_leg.timeStamp),length(left_leg.timeStamp)]);
...
for i = 1 %:t
    ...
    if(state_set(1))
        ...
        G_R_B = B_R_G';
        model.phi0 = dcm2euler(G_R_B);  %NOTE: changed in sensorsParam.m as [0 pi/2 0]
